...
for row in rows:
    ...
    if columns:
        ...
        province = columns[7].text.strip() if columns[2].text.strip() else None

        population = columns[6].text.strip().replace(',', '')  # Remove commas and spaces from the population data
        ...
        city_data = {'country': 'Belgium', 'province': province, 'flag': flag, 'city': city, 'population': population}
